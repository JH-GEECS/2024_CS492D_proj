...
class NeuralTextureEncoder(nn.Module):
    def __init__(
        ...
        for _ in range(num_res_blocks):
            self.model.append(ResBlock(dims[2]))

        # up 1
        self.model.append(
            nn.ConvTranspose2d(dims[2], dims[1], kernel_size=4, padding=1, stride=2)
        )
        self.model.append(nn.GroupNorm(num_groups=groups, num_channels=dims[1]))
        self.model.append(nn.SiLU(inplace=True))
        # up 2
        self.model.append(
            nn.ConvTranspose2d(dims[1], dims[0], kernel_size=4, padding=1, stride=2)
        )
        self.model.append(nn.GroupNorm(num_groups=groups, num_channels=dims[0]))
        self.model.append(nn.SiLU(inplace=True))
        # out
        self.model.append(nn.Conv2d(dims[0], out_dim, kernel_size=3, padding=1))
